...
p_load(tidyverse, lme4, lmerTest, mvoutlier, nlme, multcomp, lsmeans, xtable, jtools, tikzDevice, gmodels, parallel, rstan, brms)
...
## Helping function 
### RM ANOVA (just backup)
```{r, eval = F}
fit.aov <- function(form, dat){
  aov(formula = form, data = dat)
}

rmanova <- function(dv, condition = "condition", cov = NULL, ...){
  ##########
  #dv <- "nep"
  #cov = c("age", "gender")
  ##########
  ...
  iv[1] <- "time"
  iv[2] <- paste(iv_1, condition, sep = " + ")
  iv[3] <- paste(iv_2, paste("time", condition, sep = ":"), sep = " + ")
  
  if(!is_null(cov)){
  models <- paste(paste(dv, " ~ ", sep = ""), iv," + ", paste(cov, collapse = " + "),"  + Error(id)", sep = "")
    
  } else {models <- paste(paste(dv, " ~ ", sep = ""), iv," + Error(id)", sep = "")}
  
  models <- lapply(models, as.formula)
  
  fits <- lapply(models, fit.aov, ...)
  names(fits) <- models
  
  summaries <- lapply(fits, summary)
  names(summaries) <- models

  ret.list <- list("fits" = fits, "summaries" = summaries)
  
  return(ret.list)
}
```
#### Better in HLM style -> anova works
```{r}
