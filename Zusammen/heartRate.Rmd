...
info_raw <- read_excel("data/transcript.xlsx", ) 
info_raw <- info_raw %>% select(`participant nr.`, ID, date, baseStart, baseEnd, vrStart, vrEnd, Frage1, Frage2, Frage3) %>%
  mutate(baseStart = as_hms(format(baseStart, format = "%H:%M:%S")), # these lines drop the dates and leave time only.
         baseEnd = as_hms(format(baseEnd, format = "%H:%M:%S")),
         vrStart = as_hms(format(vrStart, format = "%H:%M:%S")),
         vrEnd = as_hms(format(vrEnd, format = "%H:%M:%S")), 
...
path <- "data/hr"
temp <- list.files(path =  path, patter = ".csv")
temp <- temp[temp %>% startsWith("Fitbit")]
temp <- paste(path, temp, sep = "/")
